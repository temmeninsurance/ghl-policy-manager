...
// GoHighLevel inbound-webhook URL for recaps/alerts. Optional — when the
// secret is empty or not a URL, both senders below just log and skip.
const GHL_WEBHOOK = defineSecret("GHL_WEBHOOK");

async function postToGHL(payload) {
  const url = (GHL_WEBHOOK.value() || "").trim();
  if (!url.startsWith("http")) {
    logger.warn("GHL_WEBHOOK not configured — skipping post", { type: payload.type });
    return false;
  }
  ...
  if (!resp.ok) throw new Error(`GHL webhook returned HTTP ${resp.status}`);
  return true;
}
...
// End-of-day summary posted to GHL: totals, top three, biggest deal.
exports.dailyRecap = onSchedule(
  { schedule: "0 18 * * *", timeZone: "America/Chicago", region: "us-central1", secrets: [GHL_WEBHOOK] },
  async () => {
    const today = chicagoToday();
    const snap = await getFirestore().collection("tvLeaderboard")
      .where("month", "==", today.slice(0, 7)).get();
    const rows = [];
    snap.forEach((d) => { if (Array.isArray(d.data().rows)) rows.push(...d.data().rows); });
    const todays = rows.filter((r) => r.date === today);
    if (!todays.length) {
      logger.info("dailyRecap: no sales today — nothing to post");
      return;
    }
    const byAgent = new Map();
    let revenue = 0, apps = 0, biggest = { agent: "", revenue: 0 };
    for (const r of todays) {
      const rev = Number(r.revenue) || 0;
      const a = Number(r.apps) || 0;
      revenue += rev;
      apps += a;
      const acc = byAgent.get(r.agent) || { revenue: 0, apps: 0 };
      acc.revenue += rev;
      acc.apps += a;
      byAgent.set(r.agent, acc);
      if (rev > biggest.revenue) biggest = { agent: r.agent, revenue: rev };
    }
    const top = [...byAgent].map(([agent, v]) => ({ agent, ...v }))
      .sort((a, b) => b.revenue - a.revenue).slice(0, 3);
    const money = (n) => "$" + Math.round(n).toLocaleString();
    const text =
      `📊 Temmen daily recap ${today}: ${money(revenue)} revenue on ${apps} apps. ` +
      `Top: ${top.map((t, i) => `${i + 1}) ${t.agent} ${money(t.revenue)}`).join(", ")}. ` +
      `Biggest deal: ${biggest.agent} ${money(biggest.revenue)}.`;
    await postToGHL({ type: "daily_recap", date: today, totalRevenue: revenue, totalApps: apps, top, biggestDeal: biggest, text });
    logger.info("dailyRecap posted", { revenue, apps });
  }
);

// Watchdog: alert (at most every 6h) when the sync has been stale 45+ min.
exports.syncWatchdog = onSchedule(
  { schedule: "every 30 minutes", region: "us-central1", secrets: [GHL_WEBHOOK] },
  async () => {
    const db = getFirestore();
    const meta = (await db.doc("tvLeaderboard/current").get()).data();
    const updated = meta?.updatedAt?.toDate?.();
    const ageMin = updated ? (Date.now() - updated.getTime()) / 60000 : Infinity;
    if (ageMin < 45) return;
    const stateRef = db.doc("tvAdmin/watchdog");
    const last = (await stateRef.get()).data()?.lastAlertAt?.toDate?.();
    if (last && Date.now() - last.getTime() < 6 * 3600e3) return;
    const text =
      `⚠️ Temmen leaderboard sync is STALE — last successful sync ` +
      `${updated ? Math.round(ageMin) + " minutes ago" : "never"}. Manual trigger: ` +
      `https://us-central1-temmen-leaderboard-2026.cloudfunctions.net/syncLeaderboardNow`;
    try {
      await postToGHL({ type: "alert", text, staleMinutes: Math.round(ageMin) });
    } catch (err) {
      logger.error("watchdog alert failed to send", err);
    }
    await stateRef.set({ lastAlertAt: FieldValue.serverTimestamp(), staleMinutes: Math.round(ageMin) });
    logger.warn("syncWatchdog: stale sync detected", { ageMin: Math.round(ageMin) });
  }
);
